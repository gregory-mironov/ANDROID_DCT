...
abstract class CDaoLeftovers:
    ...
    @Query("UPDATE leftovers SET _qtyout = _qtyout + 1 WHERE _gtin = :aGtin AND _sn = :aSn AND _rfid = :aRfid AND _qtyin = 0")
    abstract fun incMyQtyoutEan_13(aGtin: String, aSn: String, aRfid: String)

    @Query("SELECT * FROM leftovers WHERE _gtin = :aGtin AND _sn = :aSn AND _rfid = :aRfid AND _qtyin = 0")
